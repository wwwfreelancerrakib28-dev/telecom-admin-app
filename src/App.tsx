...
export default function AdminApp() {
  ...
  const [updateLink, setUpdateLink] = useState('[https://play.google.com/store/apps/details?id=com.telecom.app](https://play.google.com/store/apps/details?id=com.telecom.app)');
  ...
  const handleUpdateSettingsSave = () => {
    set(ref(db, 'settings/forceUpdate'), { 
  ...
  const handleApproveAddMoney = (log: any) => {
    ...
    onValue(userRef, (snapshot) => {
      ...
      if (userData) {
        const currentBal = log.balanceType === 'drive' 
          ? (userData.driveBalance || 0) 
    ...
    update(ref(db, `addMoneyLogs/${log.id}`), { 
      status: 'Approved' 
    });
      ...
      <div className="min-h-screen bg-slate-50 flex items-center justify-center p-4 font-sans text-xs">
        <div className="w-full max-w-xs bg-white rounded-3xl p-6 shadow-xl border text-center">
          ...
          >
            ...
            {authError && (
              <div className="p-2 bg-red-50 text-red-500 text-xs text-center rounded-xl">
                {authError}
              </div>
            )}

            {/* PIN INPUT: শুধু Numeric Keyboard */}
            <input
              type="text"
              inputMode="numeric"
              pattern="[0-9]*"
              ...
              autoComplete="off"
              className="w-full bg-slate-50 border rounded-xl p-2.5 text-xs text-center font-bold tracking-widest"
              style={{ 
        ...
        <div className="flex items-center gap-2.5">
          ...
          {activeSection !== 'menu' && (
            <button 
              ...
              className="p-1.5 -ml-1 rounded-xl bg-slate-100"
            >
      ...
      <main className="flex-1 p-3 max-w-lg mx-auto w-full overflow-y-auto space-y-3">
        ...
        {activeSection === 'menu' && (
          <div className="grid grid-cols-2 gap-2.5">
            ...
            <button 
              ...
              className="bg-white border rounded-2xl p-3 flex flex-col items-center text-center shadow-sm"
            >
            ...
            <button 
              onClick={() => setActiveSection('update_control')} 
              className="bg-white border rounded-2xl p-3 flex flex-col items-center text-center shadow-sm"
            >
          ...
          <div className="space-y-3">
            ...
            <div className="bg-white border rounded-2xl p-3.5 flex items-center justify-between shadow-sm">
              ...
              <button onClick={() => setAddMoneyEnabled(!addMoneyEnabled)}>
                {addMoneyEnabled 
                  ...
                  : <ToggleLeft className="w-7 h-7 text-rose-600" />
                }
              </button>
            </div>
            ...
            <div className="bg-white border rounded-2xl p-3.5 space-y-2 shadow-sm">
              ...
              <h4 className="font-bold text-slate-900 border-b pb-1.5 flex items-center gap-1">
                ...
                এড-মানি নির্দেশিকা নোট
              </h4>

              <textarea 
                value={noteInput} 
                onChange={(e) => setNoteInput(e.target.value)} 
                placeholder="ইউজার অ্যাপে দেখানোর জন্য নোট লিখুন..." 
                className="w-full bg-slate-50 border rounded-xl p-2.5 h-20 text-xs" 
              />

              <button 
                onClick={updateAddMoneyNoteInDb} 
            ...
            <div className="bg-white border rounded-2xl p-3.5 space-y-2.5 shadow-sm">
              ...
              <h4 className="font-bold text-slate-900 border-b pb-1.5">
              ...
              <input 
                ...
                className="w-full bg-slate-50 border rounded-xl p-2 text-xs font-mono font-bold" 
              />
              ...
              <button 
                onClick={updateAddMoneySettings} 
        ...
        {activeSection === 'update_control' && (
          <div className="space-y-3">
            ...
            <div className="bg-white border rounded-2xl p-3.5 flex items-center justify-between shadow-sm">
              ...
              <div>
                <h4 className="font-bold text-slate-900">
                  ফোর্স আপডেট (Force Update)
                </h4>

                <p className="text-[10px] text-slate-400">
                  চালু করলে ইউজাররা আপডেট ছাড়া অ্যাপে ঢুকতে পারবে না
                </p>
              </div>
        ...
        {activeSection === 'history' && (
          <div className="space-y-2">
            ...
            <h4 className="font-bold text-slate-800 px-1">
              এড-মানি রিকোয়েস্ট লিস্ট
            </h4>
            ...
            {addMoneyLogs.map(log => (
              ...
              <div 
                key={log.id} 
                className="bg-white border rounded-xl p-3 space-y-2 shadow-sm"
              >
                ...
                <div className="flex justify-between items-center">
                  ...
                  <div>
                    ...
                    <p className="text-[10px] text-slate-400">
                      TrxID: <strong className="font-mono text-indigo-600">
                        {log.trxId}
                      </strong>
                    </p>
                ...
                {log.status === 'Pending' && (
                  <div className="flex gap-2 pt-1 border-t">
                    ...
                    <button 
                      onClick={() => handleApproveAddMoney(log)} 
                    ...
                    <button 
                      onClick={() => handleCancelAddMoney(log.id)} 
                      className="flex-1 py-1.5 bg-rose-600 text-white font-bold rounded-lg text-xs"
                    >
                      Cancel
                    </button>
